...
function calculate_saginatoble_maximum_principal_stresses(load_scale)
    ...
    principal_compressive_stress = []
    for i in eachindex(principal_stresses)

        for j=1:3

            principal_tensile_stress = push!(principal_tensile_stress, principal_stresses[i][j][2])
            principal_compressive_stress = push!(principal_compressive_stress, principal_stresses[i][j][1])
        end

    end

    max_principal_stresses = [maximum(principal_tensile_stress), minimum(principal_compressive_stress)]

    return max_principal_stresses

end

function bridge_failure_criteria(max_principal_stresses, compressive_stress_limit, tensile_stress_limit)

    tension_failure = "no"
    compression_failure = "no"

    if (abs(max_principal_stresses[2]) > compressive_stress_limit) | (max_principal_stresses[1] > tensile_stress_limit)

        if abs(max_principal_stresses[2]) > compressive_stress_limit
            compression_failure = "yes"
        end

        if max_principal_stresses[1] > tensile_stress_limit
            tension_failure = "yes"
        end
    ...
    return bridge_failure, tension_failure, compression_failure
...
function calculate_probability_of_failure(f_c, load_scale)
    ...
    failure_count = []
    all_max_principal_stresses = []
    compression_failures = []
    tension_failures = []
    for i = 1:length(f_c)

        max_principal_stresses = calculate_saginatoble_maximum_principal_stresses(load_scale[i])

        all_max_principal_stresses = push!(all_max_principal_stresses, max_principal_stresses)

        compressive_stress_limit = 0.85 * f_c[i]
        tensile_stress_limit = 10.0 * 1/10 * f_c[i]

        bridge_failure, tension_failure, compression_failure = bridge_failure_criteria(max_principal_stresses, compressive_stress_limit, tensile_stress_limit)

        failure_count = push!(failure_count, bridge_failure)
        compression_failures = push!(compression_failures, compression_failure)
        tension_failures = push!(tension_failures, tension_failure)

    end

    num_failures = length(findall(check->check==1, failure_count))
    num_compression_failures = length(findall(check->check=="yes", compression_failures))
    num_tension_failures = length(findall(check->check=="yes", tension_failures))

    failure_probability = num_failures/num_samples 

    results = (failure_probability=failure_probability, num_compression_failures=num_compression_failures, num_tension_failures=num_tension_failures )
