...
num_dof = size(node_coordinates, 1) * 3  #3 dof per node   #update this Tank
...
### this will change, add frame element, matrix is symmetric Tank 
...
function define_global_dof_for_element(node_number_i, node_number_j)
    ...
    #change these 
    dof_i = [node_number_i * 2 - 1, node_number_i * 2, node_number_i * 2 + 1]
    dof_j = [node_number_j * 2 - 1, node_number_j * 2, node_number_j * 2 + 1]
